...
public class SheetAddService<T extends Msg> implements Service
{
	...
	// 派单追加Topic
	private static String SheetStatusAddTopic = "/Alarm/Abims/AlarmChange/SendSheet";
		...
		for(Map mo : moList)
		{
			...
			for (Map map : data)
			{
				log.debug("工单同步记录:sheetno=" + mo.get("sheetno") + ",serialno="
						...
						+ ",sheettime=" + map.get("sheettime")
						+ ",sheetstarttime=" + map.get("sheetstarttime"));
				if (!sheetpart.equals(String.valueOf(map.get("sheetpart"))))
				{
					...
					{
						sheetstarttime = Long.parseLong(String.valueOf(mo.get("sheetstarttime")));
					}
					else if(map.get("sheetstarttime")!=null && !"".equals(map.get("sheetstarttime")))
					{
						sheetstarttime = Long.parseLong(String.valueOf(map.get("sheetstarttime")));
					}
			...
			// 工单追加发派单成功消息
			Map nodeAddAttrs = new HashMap();
			nodeAddAttrs.put("sheetstatus", String.valueOf(mo.get("sheetstatus")));
			nodeAddAttrs.put("alarmuniqueid", String.valueOf(mo.get("alarmuniqueid")));
			nodeAddAttrs.put("sheetno", String.valueOf(mo.get("sheetno")));
			nodeAddAttrs.put("sheetsendfailres", "");
			nodeAddAttrs.put("sheetsendstatus", "5");
			addSubNodes.add(nodeAddAttrs);
			
			synchronized (sendMap) {
				sendMap.put(String.valueOf(mo.get("sheetno")), subNodes);
			}
			synchronized (addSendMap) {
				addSendMap.put(String.valueOf(mo.get("sheetno")), addSubNodes);
			}
		}
		return true;
	}
	...
	private Map<String,List<Map>> addSendMap = new ConcurrentHashMap<String, List<Map>>();
	class SendThread extends Thread
	{
		public void run()
		{
			Map<String,List<Map>> map = new ConcurrentHashMap<String, List<Map>>();
			Map<String,List<Map>> addMap = new ConcurrentHashMap<String, List<Map>>();
			while(true)
			{
				try {
					map.clear();
					...
					synchronized (addSendMap)
					{
						addMap.putAll(addSendMap);
						addSendMap.clear();
					}
					it = addMap.values().iterator();
					while (it.hasNext())
					{
						List<Map> subNodes = it.next();
						if (!subNodes.isEmpty())
						{
							// 发送工单变更记录
							Map data = NodeUtil.createElement("", "", null, subNodes);
							log.info("发送派单追加消息变更记录" + data);
							ac.sendMsg(SheetStatusAddTopic, data);
							Thread.sleep(50);
						}
					}
					Thread.sleep(2000);
